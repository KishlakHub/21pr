...
namespace RegIN_Subbotin.Classes
{
    public class User
    {
        ...
        public void SetUser()
        {
            ...
            if (WorkingDB.OpenConnection(mySqlConnection))
            {
                MySqlCommand mySqlCommand = new MySqlCommand("INSERT INTO `users`" +
        ...
        public void CrateNewPassword()
        {
            if (Login != String.Empty)
            {
                ...
                if (WorkingDB.OpenConnection(mySqlConnection))
                {
                    WorkingDB.Query($"UPDATE `users` SET `Password`='{this.Password}', `DateUpdate`='{this.DateUpdate.ToString("yyyy-MM-dd HH:mm:ss")}' " +
                       $"WHERE `Login` = '{this.Login}'", mySqlConnection);
                }
                WorkingDB.CloseConnection(mySqlConnection);
